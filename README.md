...
рукопожатии — показ экрана, `preconnect()`, первый запрос после `identify`, обновление сессии, —
а не при `identify`, поэтому токен запрашивается прямо перед открытием чата. 15-минутную сессию
SDK обновляет, когда делает запрос меньше чем за 60 секунд до её истечения, и отправляет
последний токен из `identify`. Пока чат открыт, вызывайте `identify` со свежим токеном не реже
раза в 5 минут и при возврате из фона: сессия, обновлённая со старым токеном, теряет инструменты
с доступом к вашим данным (связь и вебхук остаются). Токен того же `sub` ленту не очищает.
...
пустым, лента и черновик очищаются сразу. Любой токен, кроме токена уже применённого
пользователя (другой или нечитаемый `sub`, первый `identify` на установке), отвязывает прежнего и
без выхода; токен того же пользователя ленту не трогает; повторный вход со свежим токеном,
выпущенным после выхода, возвращает прежний тред. Выход пишется на диск синхронно, ещё на потоке
вызывающего, и переживает перезапуск — в том числе запрошенный до `configure`: тогда это файл в
`noBackupFilesDir`, а контекст приложения SDK получает через `androidx.startup`
(`ru.meerbot.sdk.internal.MeerBotInitializer`). Отключили инициализатор — выход до `configure`
живёт только в памяти процесса (`logout_not_persisted` в логе). Сервер, не знающий выхода, сигнал
...
val state by controller.state.collectAsState()   // messages / mode / sending / connectionError / retryable / draft
controller.setDraft("…")                         // черновик — в state.draft: смена пользователя стирает его с лентой
controller.start()
